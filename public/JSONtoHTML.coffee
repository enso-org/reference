...
conversion = (data) ->
  ...
  while i < modules.length 
    ...
    while j < modules[i].classes.length
      out += '<div class="container"><div class="pageHead" style="width:90%"><h5>' + modules[i].classes[j] + '</h5><br></div></div>'
      j++
    out += '<div class="container"><div class="pageHead" style="width:95%"><h3>Functions</h3></div></div>'
    j = 0
    while j < modules[i].functions.length
      out += '<div class="container"><div class="pageHead" style="width:90%"><h5>' + modules[i].functions[j] + '</h5><br></div></div>'
      j++
    i++
