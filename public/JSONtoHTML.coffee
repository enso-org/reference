  ...
  while i < modules.length 
    ...
    while j < modules[i].classes.length
      ...
      if modules[i].classes[j].documentation != null
        ...
        out += '<h4>' + mdtohtml + '</h4>'
      out += '</div></div>'
      menu += '<a class="btn btn-default" style="margin-left:30px" href="#' + modules[i].name + '_' + modules[i].classes[j].name + '">' + modules[i].classes[j].name + '</a><br><button class="accordion btn btn-default">+</button>
<div class="panel">'
      k = 0
    ...
    if modules[i].functions.length != 0
      ...
      menu += '<a class="btn btn-default" style="margin-left:30px" href="#' + modules[i].name + '_Functions">Functions</a><br><button class="accordion btn btn-default">+</button><div class="panel">'
      j = 0
