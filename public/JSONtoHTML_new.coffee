...
conversion = (data) ->
  ...
  while i < modules.length 
    ...
    while j < modules[i].classes.length
      ...
      out += '</div></div>'
      menu += '<a align="right" href="javascript:showhide(\''+ modules[i].name + '_' + modules[i].classes[j].name + 'Menu\')">(+)</a><a style="margin-left:30px" href="#' + modules[i].name + '_' + modules[i].classes[j].name + '">' + modules[i].classes[j].name + '</a><br>
