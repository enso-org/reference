...
conversion = (data) ->
  out = ''
  menu = '<a align="center" >Libraries:</a>'
  i = 0
  modules = data.modules
 
  while i < modules.length 
    out += '<div class="container"><div class="pageHead" id="' + modules[i].name + '"><h1>' + modules[i].name + '</h1></div></div>'
    menu += '<a class="btn btn-default" href="#' + modules[i].name + '">' + modules[i].name + '</a><a class="btn btn-default" align="right" href="javascript:showhide(\''+ modules[i].name + 'Menu\')">(+)</a><br><div id="'+ modules[i].name + 'Menu" style="display:none;">'
    ...
    while j < modules[i].classes.length
      ...
      out += '</div></div>'
      menu += '<a class="btn btn-default" style="margin-left:30px" href="#' + modules[i].name + '_' + modules[i].classes[j].name + '">' + modules[i].classes[j].name + '</a><a class="btn btn-default" align="right" href="javascript:showhide(\''+ modules[i].name + '_' + modules[i].classes[j].name + 'Menu\')">(+)</a><br>
<div id="'+ modules[i].name + '_' + modules[i].classes[j].name + 'Menu" style="display:none;">'
    ...
    if modules[i].functions.length != 0
      ...
      menu += '<a class="btn btn-default" style="margin-left:30px" href="#' + modules[i].name + '_Functions">Functions</a><a class="btn btn-default" align="right" href="javascript:showhide(\''+ modules[i].name + '_FunctionsMenu\')">(+)</a><br><div id="'+ modules[i].name + '_FunctionsMenu" style="display:none;">'
      j = 0
